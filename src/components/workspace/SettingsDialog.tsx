      ...
      <DialogContent className="sm:max-w-md max-h-[85vh]">
        ...
        <div className="space-y-6 py-2">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                ...
                <span className={`text-sm font-medium ${storageSettings.mode === "cloud" ? "text-primary" : "text-foreground"}`}>云端同步</span>
                <span className="text-[11px] text-muted-foreground text-center leading-tight">数据保存在云端，多设备同步</span>
              </button>
              <button
              ...
              >
                ...
                <span className={`text-sm font-medium ${storageSettings.mode === "local" ? "text-primary" : "text-foreground"}`}>本地存储</span>
                ...
                {isDesktop ? (
                  <>
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={handlePickDirectory} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium bg-accent text-accent-foreground hover:bg-accent/80 transition-colors">
                        <FolderOpen className="w-3 h-3" /> 选择目录
                      </button>
                    </div>
                    {storageSettings.localPath ? (
                      <p className="text-xs text-muted-foreground font-mono break-all bg-background px-2 py-1.5 rounded border border-border">{storageSettings.localPath}</p>
                    ) : (
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-[11px] text-muted-foreground leading-relaxed">当前为 Web 版本，笔记将保存在浏览器本地数据库（IndexedDB）中。</p>
                      <p className="text-[11px] text-muted-foreground leading-relaxed">如需保存为本地文件并自选目录，请使用桌面安装版。</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="space-y-3">
            ...
            <Slider value={[pageFontSize]} onValueChange={(val) => onPageFontSizeChange(val[0])} min={PAGE_FONT_MIN} max={PAGE_FONT_MAX} step={1} />
